...
| 2026-05-06 | Vitest set up (native tsconfig path resolution, no plugin). 16 tests for split-bill calculator covering happy path, input clamping, realistic scenarios, invariants. `pnpm test` and `pnpm test:watch` scripts. |
| 2026-05-06 | Privacy policy (`/privacy`) + Terms of service (`/terms`) pages drafted in plain English. Added to sitemap (priority 0.4). Footer links on home page. Required for AdSense application. |
